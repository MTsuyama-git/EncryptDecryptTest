...
namespace Utility
{
    public class SSHKeyManager
    {
        ...
        public static ReadOnlySpan<byte> ReadRSAPrivateKey(string contents, Func<string>? passwordCb = null, bool verbose = false)
        {
            ...
            {
                var h = header.Split(":", StringSplitOptions.RemoveEmptyEntries);
                if (h[0] == "DEK-Info")
                {
                    encryptInfo = h[1].Split(",", StringSplitOptions.RemoveEmptyEntries)[0].Replace(" ", String.Empty).Split('-', StringSplitOptions.RemoveEmptyEntries);
                    string ivInfo = h[1].Split(",", StringSplitOptions.RemoveEmptyEntries)[1].Replace(" ", String.Empty);
