...
namespace Utility
{
    public class Bcrypt
    {
        ...
        static byte[] hash(ref byte[] pass, ref byte[] salt)
        {
            ...
            Type? cdataType = cdata.GetType().GetElementType();
            if(cdataType == null) {
                throw new Exception("Unknown cdata");
            }
            int sizeofCdata = System.Runtime.InteropServices.Marshal.SizeOf(cdataType) * cdata.Length;
            for (int i = 0; i < 64; ++i)
        ...
        public static int pbkdf(string password, byte[] salt, ref byte[] key, int rounds)
        {
            ...
            Type? saltType = salt.GetType().GetElementType();
            if(saltType == null) {
                throw new Exception("Unknown salt");
            }
            int typeSize = System.Runtime.InteropServices.Marshal.SizeOf(saltType);
            ...
            for (int count = 1; keylen > 0; count++)
            {
                ...
                if(outputType == null)
                {
                    throw new Exception("Unkonwn output");
                }
                Buffer.BlockCopy(tmpoutput, 0, output, 0, System.Runtime.InteropServices.Marshal.SizeOf(outputType) * output.Length);
                for (i = 1; i < rounds; ++i)
                {
